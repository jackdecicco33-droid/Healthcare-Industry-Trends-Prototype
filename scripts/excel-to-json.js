...
const workbookDataModulePath = path.join(root, 'data', 'workbook-data.js');
...
function writeWorkbookDataModule(resources, terminology, signals) {
  const moduleSource = [
    ...
    `export const workbookResources = ${JSON.stringify(resources, null, 2)};`,
    `export const workbookTerminology = ${JSON.stringify(terminology, null, 2)};`,
    `export const workbookHealthcareSignals = ${JSON.stringify(signals, null, 2)};`,
    ''
  ].join('\n\n');

  fs.writeFileSync(workbookDataModulePath, moduleSource, 'utf8');
}
...
function main() {
  ...
  writeWorkbookDataModule(resources, terminology, signals);
  ...
  console.log(`Wrote frontend workbook data module to ${path.relative(root, workbookDataModulePath)}`);
}
